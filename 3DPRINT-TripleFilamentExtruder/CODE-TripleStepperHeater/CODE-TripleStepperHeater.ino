...
void loop() {
  //--------Analog potentiometer to speed-----
  int sensorValuepot = analogRead(A3);
  speedpot = map(sensorValuepot, 0, 1023, 0, 5000);

  int stepDelayE0= speedpot;
  
  //--------PID TEMPERATURE-----

  setPoint0 = 185; // 220 DEGREES
 ...
 // myPID2.run(); //call every loop, updates automatically at certain time interval
  analogWrite(MOSFET_D8_PIN, 255);        // COOLING FAN FOR HEATSINKS
  ...
  if (reading1 != lastoctocoupler1State) {
            ...
            if ((millis() - lastDebounceTime) > debounceDelay) {
              ...
              // delay, so take it as the actual current state:
                     ...
                     if ( temp0 <= (setPoint0-30)){                         // prevent cold extrude
      ...
      //--------SERIAL PRINT DEBBUGGING-----//
        Serial.print("Temp:"); 
           ...
           Serial.print(outputVal0);
           Serial.print("-- RobSig:");
           Serial.print(octocoupler1State);
           Serial.print("-- Speed:");
           Serial.print(speedpot);
           Serial.print("-- Speedelay:");
           Serial.println(stepDelayE0);
           //Serial.flush();
  /*
  if (octocoupler1 ==1){
